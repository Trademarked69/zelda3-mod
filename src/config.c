...
static bool HandleIniConfig(int section, const char *key, char *value) {
  if (section == 0) {
    ...
    } else if (StringEqualsNoCase(key, "BilinearFiltering")) {
      return ParseBool(value, &g_config.bilinear_filtering);
    ...
    if (StringEqualsNoCase(key, "Autosave")) {
      ...
      while ((s = NextDelim(&value, ',')) != NULL) {
        ...
        else if (strcmp(s, "30:17") == 0)
          g_config.extended_aspect_ratio = (h * 30 / 17 - 256) / 2;
